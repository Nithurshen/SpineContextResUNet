...
class ContextBlock_LiDilation(nn.Module):
    """
    ABLATION VERSION: Linear.
    Provides smoother, incremental context expansion compared to exponential dilation.
    """
    def __init__(self, in_channels, out_channels):
        super(ContextBlock_LiDilation, self).__init__()
        self.d1 = nn.Conv3d(in_channels, out_channels, kernel_size=3, padding=1, dilation=1)
        self.d2 = nn.Conv3d(in_channels, out_channels, kernel_size=3, padding=2, dilation=2) 
        self.d4 = nn.Conv3d(in_channels, out_channels, kernel_size=3, padding=3, dilation=3)
        self.d8 = nn.Conv3d(in_channels, out_channels, kernel_size=3, padding=4, dilation=4)
...
class SpineResUNet_LiDilation(nn.Module):
    def __init__(self, in_channels=1, out_channels=1, base_filters=16):
        super(SpineResUNet_LiDilation, self).__init__()
        ...
        self.bottleneck = ContextBlock_LiDilation(base_filters * 4, base_filters * 8)
