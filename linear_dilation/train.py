...
def train():
    ...
    vis_dir = "visualizations_li_dilation"
    os.makedirs(vis_dir, exist_ok=True)
    print(f"--> Visualizations will be saved to: {vis_dir}/")

    model = SpineResUNet_LiDilation(in_channels=1, out_channels=1).to(DEVICE)
    ...
    os.makedirs("checkpoints_li_dilation", exist_ok=True)
    ...
    for epoch in range(EPOCHS):
        ...
        if avg_val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(), "models/best_model_linear_dilation.pth")
            print("  --> New Best Model Saved!")
            print("-" * 30)

        torch.save(model.state_dict(), f"checkpoints_li_dilation/model_epoch_{epoch + 1}.pth")
