...
def train():
    ...
    vis_dir = "visualizations_context_off"
    os.makedirs(vis_dir, exist_ok=True)
    print(f"--> Visualizations will be saved to: {vis_dir}/")

    model = SpineResUNet_cotext_off(in_channels=1, out_channels=1).to(DEVICE)
    ...
    os.makedirs("models_context_off", exist_ok=True)
    os.makedirs("checkpoints_context_off", exist_ok=True)
    ...
    for epoch in range(EPOCHS):
        ...
        if avg_val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(), "models/best_model_context_off.pth")
