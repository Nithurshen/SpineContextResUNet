...
class Plain3DUNet(nn.Module):
    ...
    def forward(self, x):
        ...
        if d1.shape != x4.shape:
            d1 = F.interpolate(
                d1, size=x4.shape[2:], mode="trilinear", align_corners=False
            )
        ...
        if d2.shape != x3.shape:
            d2 = F.interpolate(
                d2, size=x3.shape[2:], mode="trilinear", align_corners=False
            )
        ...
        if d3.shape != x2.shape:
            d3 = F.interpolate(
                d3, size=x2.shape[2:], mode="trilinear", align_corners=False
            )
        ...
        if d4.shape != x1.shape:
            d4 = F.interpolate(
                d4, size=x1.shape[2:], mode="trilinear", align_corners=False
            )
