...
# --- CONFIGURATION ---
# TotalSegmentator handles its own device management (CPU/MPS)
...
def run_test():
    ...
    # We test on GPU (MPS) if available, then CPU
    devices_to_test = ["cpu"]
    if torch.backends.mps.is_available():
        devices_to_test.append("mps") # TotalSegmentator API uses "gpu" for MPS
    ...
    for dev in devices_to_test:
        ...
        # TotalSegmentator creates individual files for each organ
        with tempfile.TemporaryDirectory() as tmp_dir:
            try:
                # Task 'total' includes vertebrae
                totalsegmentator(
                    VOL_PATH, 
                    tmp_dir, 
                    task="total", 
                ...
                # Aggregate all vertebrae files into one mask
