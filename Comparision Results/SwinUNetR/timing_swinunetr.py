...
# Update path to find the SwinUNETR model
sys.path.append('Comparision Results/SwinUNetR')
        ...
        for z in z_steps:
            for y in y_steps:
                for x in x_steps:
                    ...
                    # SwinUNETR typically outputs logits; apply sigmoid
...
def run_test():
    ...
    for dev_type in devices:
        ...
        print(f"\n--- Testing SwinUNETR-Nano on {dev_type.upper()} ---")
        
        # Initialize model
        model = SwinUNETR_Nano(in_channels=1, out_channels=1)
        ...
        if dev_type == "mps":
            ...
            dummy = torch.randn(1, 1, 128, 128, 64).to(device)
            _ = model(dummy)
