...
from no_dilation.model import SpineResUNet_NoDilation
...
MODEL_PATH = "models/best_model_no_dilation.pth"

TEST_VOL_DIR = "data/raw/CTSpine1k/volumes/test"
TEST_SEG_DIR = "data/raw/CTSpine1k/labels/test"
RESULTS_DIR = "results/no_dilation/ctspine1k_test"
            ...
            for y in y_steps:
                for x in x_steps:
                    ...
                    if (curr_d, curr_h, curr_w) != PATCH_SIZE:
                        ...
                        pad_h = ph - curr_h
                        pad_w = pw - curr_w
                        slice_vol = F.pad(slice_vol.unsqueeze(0).unsqueeze(0), 
                                          (0, pad_w, 0, pad_h, 0, pad_d)).squeeze()
...
def save_visual(ct_vol, pred_mask, subject_id, output_dir):
    ...
    ax.set_title(f"Prediction: {subject_id}", fontsize=14, color="white", backgroundcolor="black")
    ax.axis("off")
...
def run_evaluation():
    ...
    model = SpineResUNet_NoDilation().to(DEVICE)
    ...
    if detailed_results:
        ...
        print(f"Best Case       : {np.max(dices):.4f} ({detailed_results[np.argmax(dices)]['ID']})")
        print(f"Worst Case      : {np.min(dices):.4f} ({detailed_results[np.argmin(dices)]['ID']})")
        print("=" * 45)
