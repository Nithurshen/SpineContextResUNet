...
from model import Plain3DUNet  # Updated import
import os
...
def save_visualization(epoch, img_tensor, mask_tensor, pred_tensor, save_dir):
    ...
    ax[2].set_title(f"Prediction (Plain UNet)")
    ax[2].axis("off")
...
def train():
    print(f"--- Starting Training (Plain 3D UNet) on {DEVICE} ---")
    ...
    # Renamed output directories to separate from previous experiments
    vis_dir = "visualizations_plain"
    os.makedirs(vis_dir, exist_ok=True)
    print(f"--> Visualizations will be saved to: {vis_dir}/")

    # Instantiating Plain 3D UNet
    model = Plain3DUNet(in_channels=1, out_channels=1, base_filters=18).to(DEVICE)
    ...
    os.makedirs("checkpoints_plain", exist_ok=True)
    ...
    for epoch in range(EPOCHS):
        ...
        if avg_val_loss < best_val_loss:
            ...
            # Renamed best model file
            torch.save(model.state_dict(), "models/best_model_plain.pth")
