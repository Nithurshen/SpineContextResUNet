...
class SpineGradCAM:
    ...
    def compute_patch(self, input_tensor):
        ...
        heatmap = F.interpolate(
            heatmap, size=input_tensor.shape[2:], mode="trilinear", align_corners=False
        )

        return output.detach(), heatmap.detach()


def predict_and_explain_sliding_window(model, vol, grad_cam):
    d, h, w = vol.shape
    pd, ph, pw = PATCH_SIZE

    prob_map = torch.zeros(vol.shape).to(DEVICE)
    cam_map = torch.zeros(vol.shape).to(DEVICE)
    weight_map = torch.zeros(vol.shape).to(DEVICE)
    ...
    for z in z_steps:
        for y in y_steps:
            for x in x_steps:
                ...
                pred_patch = pred_patch.squeeze()
                cam_patch = cam_patch.squeeze()
    ...
    avg_cam = cam_map / weight_map

    return avg_prob.cpu().numpy(), avg_cam.cpu().numpy()


def save_dual_visual(ct_vol, pred_mask, cam_vol, subject_id, output_dir):
    ...
    if robust_max == 0:
        robust_max = cam_vol.max()  # Fallback
    ...
    ax[1].set_title(
        f"Grad-CAM Attention: {subject_id}",
        fontsize=14,
        color="white",
...
def run_evaluation():
    ...
    grad_cam = SpineGradCAM(model, model.bottleneck)
    ...
    print(f"--- Analyzing {len(ct_files)} Volumes ---")
    ...
    for ct_path in tqdm(ct_files, desc="Processing"):
        ...
        pred_prob_vol, cam_vol = predict_and_explain_sliding_window(
            model, ct_vol, grad_cam
        )

        pred_bin_vol = (pred_prob_vol > 0.5).astype(np.float32)
        pred_bin_vol = keep_largest_blob(pred_bin_vol)

        dice = compute_dice(pred_bin_vol, gt_vol)
        detailed_results.append({"ID": subject_id, "Dice": dice})

        save_dual_visual(ct_vol, pred_bin_vol, cam_vol, subject_id, RESULTS_DIR)
    ...
    pd.DataFrame(detailed_results).to_csv(
        os.path.join(RESULTS_DIR, "test_metrics_cleaned.csv"), index=False
    )
    print("Results saved.")
